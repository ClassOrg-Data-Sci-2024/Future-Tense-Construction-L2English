...
```{r}
future_tokens_data %>% count(level_id)
```
...
```{r}
yrs_in_english_environment_intercepts_model <- lmer(is_will_construction ~ (1 | anon_id) + yrs_in_english_environment , data = future_tokens_data)

yrs_in_english_environment_model_coefs = coef(yrs_in_english_environment_intercepts_model)$anon_id %>% 
  rename(Intercept = `(Intercept)`) %>% 
  rownames_to_column("anon_id")

yrs_in_english_environment_model_coefs %>% 
  left_join(student_info %>% select(anon_id, yrs_in_english_environment, native_language), by = 'anon_id') %>% 
  group_by(native_language) %>% filter(n()>30) %>% 
  ggplot()+
  geom_boxplot(aes(x = yrs_in_english_environment, y = Intercept)) +
  facet_wrap(~native_language)
```


```{r}
time_eng_intercepts_model <- lmer(is_will_construction ~ (1 | anon_id) + yrs_in_english_environment +  yrs_of_english_learning, data = future_tokens_data)

time_eng_model_coefs = coef(time_eng_intercepts_model)$anon_id %>% 
  rename(Intercept = `(Intercept)`) %>% 
  rownames_to_column("anon_id")

yrs_in_english_environment_model_coefs %>% 
  left_join(student_info %>% select(anon_id, yrs_in_english_environment, native_language, yrs_of_english_learning), by = 'anon_id') %>% 
  group_by(native_language) %>% filter(n()>30) %>% 
  ggplot()+
  geom_boxplot(aes(x = yrs_in_english_environment, y = Intercept)) +
  facet_wrap(~yrs_of_english_learning)
```
...
```{r}
t = level_id.model_coefs %>% 
  select(anon_id,Intercept) %>% 
  left_join(future_tokens_data %>% select(anon_id, level_id), by = 'anon_id') %>% 
  mutate(level_id = factor(level_id)) %>% 
  distinct()

summary(aov(Intercept ~ level_id, data = t))
```
